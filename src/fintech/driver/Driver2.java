    ...
    public static void main(String[] _args) {
        ...
        while (true) {
            ...
            if (transactionToRevert != null) {
                // to check if it is already over 10 mins
                long diff = System.currentTimeMillis() - Transaction.convertStringToDate(transactionToRevert.getPosted_at()).getTime();
                ...
                if (diff <= tenMinutesInMillis) {
                    ...
                    if (transactionToRevert instanceof TransferTransaction) {
                        ...
                        if(accounts.get(recipientName).getBalance()- amount >= 0){
                            accounts.get(senderName).setBalance(accounts.get(senderName).getBalance() + amount);
                            accounts.get(recipientName).setBalance(accounts.get(recipientName).getBalance() - amount);
                        }
                    }
                    else {
                        if(accounts.get(accountId).getBalance() - amount >= 0){
                    ...
                    if(isPositive){ // if the original transaction is already negative, we still need to record the positive amount
                        Transaction newTransaction = new Transaction(accountId, Math.abs(amount), data[3], "REVERT: "+ transactionToRevert.getNote());
                        addTransaction(newTransaction);
                    }
                    else{
                        Transaction newTransaction = new Transaction(accountId, -Math.abs(amount), data[3], "REVERT: "+ transactionToRevert.getNote());
